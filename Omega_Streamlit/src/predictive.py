...
def fit_regression_model(
    ...
    try:
        ...
        # Compute prediction lists to prevent KeyErrors in sandbox scripts
        preds = (A @ beta).tolist()
        # Build formatted OLS summary string for text report outputs
        summary_lines = [
            "="*60,
            f"OLS Regression Results: Target Variable '{target_col}'",
            "="*60,
            f"R-squared:         {r_squared:.4f}",
            f"Intercept:         {intercept:.4f}",
            f"Sample Size:       {n}",
            "-"*60,
            f"{'Feature':<35}{'Coefficient':>25}",
            "-"*60,
            f"{'const':<35}{intercept:>25.4f}"
        ]
        for feat, coef in coefficients.items():
            summary_lines.append(f"{feat:<35}{coef:>25.4f}")
...
def fit_classification_model(
    ...
    try:
        ...
        if not is_multiclass:
            ...
            pred_list = [str(class_1) if p >= 0.5 else str(class_0) for p in probs]
            # Build formatted classification summary string
            summary_lines = [
                "="*60,
                f"Binary Logistic Regression Results: Target Variable '{target_col}'",
                "="*60,
            ...
            pred_list = [str(x) for x in y_pred]
            # Build formatted multiclass classification summary string
            summary_lines = [
                "="*60,
                f"Multiclass Logistic Regression Results: Target Variable '{target_col}'",
                "="*60,
                f"Accuracy:          {accuracy:.4f}",
                f"Macro Precision:   {macro_precision:.4f}",
                f"Macro Recall:      {macro_recall:.4f}",
                f"Macro F1 Score:    {f1_score:.4f}",
                f"Sample Size:       {n}",
                "-"*60,
                "Classes: " + ", ".join([str(x) for x in unique_targets]),
                "="*60
            ]
            summary_str = "\n".join(summary_lines)
